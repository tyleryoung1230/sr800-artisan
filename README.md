...
2. **Config → Device**
   ...
   - Port: ESP COM @ **115200**  
3. For a **single probe**:
   - **BT** = thermocouple **1** or **2** (firmware mirrors BT on both)
   - **ET** = **None**, or also **1**/**2** if Artisan blanks BT when ET is None (both will show the same chamber temp — fine on a fluid bed)
   - After flashing current firmware, `READ` should look like `amb,BT,BT,0,0,heat,fan` so either BT channel works.  
